...
func TestManagedTerminalExactRequestOutputAndIdempotency(t *testing.T) {
	...
	_, err = terminal.SignalForeground(ManagedTerminalSignalInterrupt)
	require.ErrorIs(t, err, ErrManagedTerminalInactive)
...
func TestManagedTerminalResizeForegroundAndSignals(t *testing.T) {
	...
	signaledGroup, err := signalTerminal.SignalForeground(ManagedTerminalSignalInterrupt)
	require.NoError(t, err)
	require.Equal(t, signalTerminal.pid, signaledGroup)
